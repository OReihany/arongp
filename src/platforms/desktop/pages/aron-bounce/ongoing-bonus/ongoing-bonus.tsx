...
export class OngoingBonus extends PureComponent {
	...
	componentDidMount() {
		let mount = true;
		if (mount){
			this.setState({data: this.bounceService.data.filter(item => item.type === 0 && item.showStatus === 0) || []})
		}
		AOS.init({
			duration: 1000,
		});
		...
		return <div className={styles.onGoing}>
			...
			<Container className={styles.container}>
				<div className={styles.onGoingWrapper}>
					<h2 data-aos="fade-up" data-aos-offset="100" data-aos-delay="5" data-aos-duration="500" data-aos-easing="ease-in-out" data-aos-mirror="true">{__('Aron Groups Broker Ongoing Bonus')}</h2>
